...
const update_product = () => {
  if (
    ...
    toast.error(
      "Looks like there's no supplier selected, please select first",
      {
        autoClose: 1000,
        transition: toast.TRANSITIONS.FLIP,
        position: toast.POSITION.TOP_RIGHT,
...
<template>
        ...
        <div class="flex flex-wrap -m-4">
          ...
          <div v-else class="p-4 md:w-1/4" v-for="(product, index) in products.data" :key="index">
            <div class="h-full border-2 border-gray-100 shadow-md rounded-lg overflow-hidden group">
              <div class="relative">
                <img v-if="product.product.product_image"
                  class="lg:h-36 md:h-36 w-full object-scale-down object-center bg-gray-100"
                  :src="product.product.product_image" :alt="product.product.name" />
                <img v-if="!product.product.product_image"
                  class="lg:h-36 md:h-36 w-full object-scale-down object-center bg-gray-100"
                  src="https://dummyimage.com/720x400" :alt="product.product.name" />
                <div class="absolute hidden group-hover:block top-0 right-0 text-white p-2 rounded">
                  <button @click="quantity_open_modal(product)"
                    class="p-2 bg-orange-400 rounded-lg hover:bg-orange-600 w-auto mt-2 mr-2" title="Send to order list">
                  ...
                  <button @click="update_open_product(product)"
                    class="p-2 bg-orange-400 rounded-lg hover:bg-orange-600 w-auto mt-2 mr-2" title="Update product">
              ...
              <div class="p-4">
                ...
                <div class="flex gap-2">
                  ...
                  <p class="leading-relaxed text-xs flex gap-2 mb-2">
                    <img class="w-10 h-10 rounded-full" :src="product.supplier.image"
                ...
                <p class="leading-relaxed text-xs flex gap-2 mb-2">
                  ...
                  <img class="w-8 h-8 rounded-full" :src="product.product.user.profile_photo_url"
                ...
                <div class="gap-1">
                  ...
                  <p class="flex leading-relaxed mt-2 mb-1 text-xs break-words" :title="date_time(product.created_at)">
  ...
  <ConfirmDialogModal :show="quantityModal" @close="quantityModal = false" maxWidth="2xl">
    <template #title>
      You are about to order ({{ form.product.product.name }}) to your supplier
      ({{ form.product.supplier.supplier_name }}). Please input your desired
      quantity.</template>
    <template #content>
  ...
  <JetDialogModal :show="update_modal" @close="update_modal = false" maxWidth="2xl">
    ...
    ">
        ...
        <div class="col-span-1 my-auto flex">
          ...
          <button @click="search_results_products_to_add = true"
