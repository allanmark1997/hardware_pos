...
<template>
  <div class="flex justify-between mt-2">
    ...
    <Button class="bg-green-400 hover:bg-green-600 hover:text-white">Download</Button>
  </div>
                  ...
                  <table class="w-full text-xs text-left text-gray-500">
                    ...
                    <tbody>
                      <template v-for="(delivery_detail, key2) in delivery.details" :key="key2">
                        <tr class="bg-white border-">
                          <td class="px-1 py-1">{{ delivery_detail.product.name }}</td>
                          <td class="px-1 py-1">{{ delivery_detail.quantity }}</td>
                          <td class="px-1 py-1">{{convert_money(delivery_detail.price) }}</td>
                          <td class="px-1 py-1"><small class="bg-orange-400 rounded-sm p-[1px] text-white">
                              {{
                                delivery_detail.status == 1 ? "Success" : "Unsuccessful"
                              }}
                            </small></td>
                          <td class="px-1 py-1"> {{ date_time(delivery_detail.created_at) }}</td>
                        </tr>
                      </template>
                    </tbody>
                    </table>
