...
const openFile = () => {
  if (post_image.value.length >= 0) {
    post_image.value.splice(0, 1);
    form.image = null;
  ...
  let hidden = document.getElementById("profile_photo");
    ...
    hidden.onchange = (e) => {
      for (let index = 0; index < e.target.files.length; index++) {
        post_image.value.push(window.URL.createObjectURL(e.target.files[index]));
        form.image = e.target.files[index];
      }
    };
};
    ...
    <JetDialogModal :show="addModal" @close="addModal = false" maxWidth="2xl">
      ...
      <template #content>
        ...
        <div class="grid grid-cols-12 gap-2">
          ...
          <div v-if="post_image.length >= 0" class="col-span-12">
            <label @click="openFile"
              ...
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                <svg aria-hidden="true" class="w-7 h-7 mb-1 text-gray-400" fill="none" stroke="currentColor"
              ...
              <input id="profile_photo" type="file" accept="image/png, image/jpeg, image/jpg" class="hidden" />
            </label>
          </div>
