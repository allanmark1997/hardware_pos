        ...
        <section class="text-gray-600 p-10">
          <div class="container px-5  mx-auto">
            ...
            <div class="flex flex-wrap -m-4">
              <div v-for="(user, index) in props.users.data.filter(user => user.type != 0 && props.users.data.length != 0)" :key="index" class="xl:w-1/3  md:w-1/2 p-4">
                <div class="bg-gray-50 border border-1 border-yellow-400 shadow-xl p-6 rounded-lg">
                  <img class="h-40 rounded w-full object-cover object-center mb-6" :src="user.profile_photo_url" :alt="user.name"
                    alt="content">
                  <h3 class="tracking-widest text-indigo-500 text-xs font-medium title-font">{{ user.email }}</h3>
                  <h2 class="text-lg text-gray-900 font-medium title-font mb-4"> {{ user.name }}</h2>
                  ...
                  <p v-if="user.type != 0" class="leading-relaxed font-bold mt-5 mb-2 text-base">Controls</p>
                  ...
                  <div v-if="user.type != 0"
                    ...
                    <button @click="function_open_modal_confirmation(true, user)"
                      class="p-2  bg-green-400 rounded-lg hover:bg-green-600 " v-if="user.status == 0 && user.type != 0">
                      Activate
                    </button>
              ...
              <div v-if="props.users.data.length == 0" class="mx-auto text-center">
