        ...
        <section class="text-gray-600 p-10">
          <div class="container px-5 mx-auto">
            <div class="flex flex-wrap -m-4">
              <div v-for="(supplier, index) in props.suppliers.data" :key="index" class="xl:w-1/3 md:w-1/2 p-4">
                <div class="bg-gray-50 border border-1 border-yellow-400 shadow-xl p-6 rounded-lg group">
                  <div class="">
                    <Button @click="
                  ...
                  <img v-if="supplier.image == null" class="h-40 rounded max-w- object-fit object-center mb-6 mx-auto"
                    src="./../../../image/logo_main.png" alt="user image" />
                  <img v-else class="h-40 rounded max-w- object-fit object-center mb-6 mx-auto" :src="supplier.image"
                    :alt="supplier.name" alt="content" />
                  ...
                  <div v-if="supplier.type != 0" class="text-sm mt-2 flex justify-center w-100 whitespace-nowrap">
                    <button @click="function_open_modal_confirmation(true, supplier)"
                      class="p-2 bg-green-400 rounded-lg hover:bg-green-600"
                      v-if="supplier.status == 0 && supplier.type != 0">
                      Activate
                    </button>
                    <button @click="function_open_modal_confirmation(false, supplier)"
              ...
              <div v-if="props.suppliers.data.length == 0" class="mx-auto text-center">
  ...
  <JetDialogModal :show="updateModal" @close="updateModal = false" maxWidth="2xl">
    ...
    <template #content>
      ...
      <div class="grid grid-cols-12 gap-2">
        ...
        <JetInputError :message="form_update.errors.image" class="mt-2 col-span-12" />
        <div class="col-span-12">
          <Input label="Supplier's name" type="text" class="mt-1 block w-full" v-model="form_update.supplier_name"
            autofocus />
          <JetInputError :message="form_update.errors.supplier_name" class="mt-2" />
        </div>
        <div class="col-span-12">
          <Input label="Contact No" type="text" class="block w-full" v-model="form_update.mobile_no" autofocus />
          <JetInputError :message="form_update.errors.mobile_no" class="mt-2" />
        </div>
        <div class="col-span-12">
          <Input label="Supplier's Address" type="text" class="block w-full" v-model="form_update.address" autofocus />
    ...
    <template #footer>
      ...
      <Button :class="{ 'opacity-25': form_update.processing }" :disabled="form_update.processing"
        @click="function_update_details" class="bg-yellow-200 hover:bg-yellow-400">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"
          stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
