        ...
        <div class="flex flex-wrap -m-4">
          ...
          <div v-else class="p-4 md:w-1/4" v-for="(product, index) in products.data" :key="index">
            <div class="h-full border-2 border-gray-100 shadow-md rounded-lg overflow-hidden group ">
              ...
              <img v-if="product.product_image" class="lg:h-48 md:h-36 w-full object-cover object-center"
                :src="product.product_image" :alt="product.name">
              <img v-if="!product.product_image" class="lg:h-48 md:h-36 w-full object-cover object-center"
                src="https://dummyimage.com/720x400" :alt="product.name">
              <div class="p-6">
                <h1 class="title-font text-lg capitalize font-bold text-gray-900">{{ product.name }}</h1>

                <h2 class="tracking-widest text-lg title-font font-bold text-gray-500 mb-3">{{
                  convert_money(product.current_price.price) }}</h2>
                <div class="flex justify-between">
                  <h2 class="tracking-widest text-sm title-font font-bold text-yellow-500 mb-1">Stocks: {{
                    product.quantity }}</h2>
                  <h2 class="tracking-widest text-sm title-font font-bold text-orange-500 mb-1">Discount: {{
                    product.current_discount.discount }}%</h2>
                </div>
                <p class="leading-relaxed mb-3 text-xs truncate"><span class="font-bold text-sm">Description: </span> {{
                  product.description
                }}</p>
                <p class="leading-relaxed mb-3 text-xs truncate"><span class="font-bold text-sm">Remarks: </span> {{
                  product.remarks }}</p>
                ...
                <p class="leading-relaxed text-xs"><span class="font-bold text-sm">Updated by: </span> {{
                  product.user.name
                }}
                </p>
                <div class="flex">
                  <p class="leading-relaxed  text-xs truncate" :title="date_time(product.updated_at)"><span
                      class="font-bold text-xs">Updated at: </span> {{
                        date_time(product.updated_at) }}</p>
                  <p class="leading-relaxed mb-3 text-xs truncate" :title="date_time(product.created_at)"><span
                      class="font-bold text-xs">Created at: </span> {{
                        date_time(product.created_at) }}</p>
                </div>
